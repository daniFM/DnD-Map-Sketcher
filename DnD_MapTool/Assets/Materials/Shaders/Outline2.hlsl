...
void Outline_float(float2 UV, float OutlineThickness, float OutlineThickness2, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, out float4 Out)
{
    float halfScaleFloor = floor(OutlineThickness * 0.5);
    float halfScaleCeil = ceil(OutlineThickness * 0.5);
    float halfScaleFloor2 = floor(OutlineThickness2 * 0.5);
    float halfScaleCeil2 = ceil(OutlineThickness2 * 0.5);
    ...
    uvSamples2[0] = UV - float2(Texel.x, Texel.y) * halfScaleFloor2;
    uvSamples2[1] = UV + float2(Texel.x, Texel.y) * halfScaleCeil2;
    uvSamples2[2] = UV + float2(Texel.x * halfScaleCeil2, -Texel.y * halfScaleFloor2);
    uvSamples2[3] = UV + float2(-Texel.x * halfScaleFloor2, Texel.y * halfScaleCeil2);

    for (int i = 0; i < 4; i++)
    {
        depthSamples[i] = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, uvSamples2[i]).r;
